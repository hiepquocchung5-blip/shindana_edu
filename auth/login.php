...
// =========================================================
// FIX: Removed isset($_POST['login_btn']) because Alpine.js disables the button on submit, 
// which prevents the browser from sending the button's name in the POST payload.
    ...
    <div class="w-full max-w-5xl bg-white rounded-[32px] md:rounded-[40px] shadow-[0_20px_60px_rgba(0,0,0,0.1)] overflow-hidden flex flex-col lg:flex-row min-h-auto lg:min-h-[600px] border border-slate-100">
        ...
        <div class="w-full lg:w-1/2 p-8 sm:p-10 lg:p-12 flex flex-col justify-center bg-white relative z-20" x-data="{ showPassword: false, isSubmitting: false }">
            <div class="max-w-sm mx-auto w-full">
                ...
                <?php if(!empty($error)): ?>
                    <div class="bg-red-50 border border-red-100 text-red-600 p-4 rounded-2xl text-xs font-bold mb-6 flex items-center gap-3 animate-in slide-in-from-top-2 shadow-sm" x-init="isSubmitting = false">
                ...
                <form action="<?= route('auth/login') ?>" method="POST" class="space-y-5" @submit="isSubmitting = true">
                    ...
                    <div class="pt-2">
                        <!-- FIX: Removed name="login_btn" since Alpine :disabled strips it from payload -->
